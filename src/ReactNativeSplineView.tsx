...
const NativeView: React.ComponentType<SplineViewProps> = requireNativeView('ReactNativeSpline')
  ...
  React.useEffect(() => {
    ...
    async function subscribe() {
      ...
      subscription = Gyroscope.addListener((measurement) => {
        ...
        const distanceGain = clamp(((configs[0]?.distance ?? 1000) as number) / 1000, 0.35, 1.25)
...
export default function SplineView(props: SplineViewProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally resets the load state whenever the scene URL changes.
  React.useEffect(() => {
    setIsLoaded(false)
  }, [url])
